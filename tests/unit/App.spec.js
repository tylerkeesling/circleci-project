...
describe("App.vue", () => {
  ...
  xit("should start with 0 clicks", () => {
    const wrapper = createWrapper();
    const count = wrapper.get("#count");
    expect(count.text()).toBe("0");
  });

  xit("should increment count and show '1' when button is clicked once", async () => {
    const wrapper = createWrapper();
  ...
  xit("should increment count and show '2' when button is clicked twice", async () => {
    const wrapper = createWrapper();
    const btn = wrapper.get("#increment-btn");
    const beforeCount = wrapper.get("#count");
    expect(beforeCount.text()).toBe("0");
